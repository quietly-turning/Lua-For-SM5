	...
	constructor(props){
		...
		$.ajax({
			...
			$.ajax({
				...
				const actors_doc       = $(lua_documentation).children().find("Classes")

				const actors           = Array.from($(lua_dot_xml).children().find("Classes Class"))
				...
				actors.forEach(function(actor_class){
					...
					const class_doc  = $(actors_doc).find("Class[name=" + class_name + "]")
